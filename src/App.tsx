...
export default function App() {
  ...
  useEffect(() => registerRadioTools({ configure: (topic, mode, mood) => setSettings({ topic, ...(mode ? { mode } : {}), ...(mood ? { mood } : {}) }), start, stop, read: () => ({ status: snapshot.current.status, programTitle: snapshot.current.programTitle, segmentTitle: snapshot.current.segmentTitle, line: snapshot.current.currentLine }) }), [setSettings, start, stop])
  return (
    <main className="min-h-screen bg-ink text-slate-100">
      <header className="mx-auto flex max-w-[1380px] items-center justify-between px-5 py-5 lg:px-8">
        ...
        <div className="flex items-center gap-3"><span className="hidden text-xs text-slate-500 sm:block">{radio.notice}</span><span className={`mode-badge ${radio.settings.workerUrl ? 'ai' : ''}`}>{radio.settings.workerUrl ? 'AI MODE' : 'DEMO MODE'}</span></div>
      </header>
      <div className="mx-auto grid max-w-[1380px] gap-5 px-5 pb-10 lg:grid-cols-[minmax(0,1.55fr)_minmax(310px,.65fr)] lg:px-8">
        <div className="space-y-5"><RadioConsole status={radio.status} line={radio.currentLine} programTitle={radio.programTitle} segmentTitle={radio.segmentTitle} elapsed={radio.elapsed} onStart={radio.start} onStop={radio.stop} onPause={radio.togglePause} onNext={radio.nextTopic} onContinue={radio.continueTopic} /><Transcript lines={radio.history} /></div>
