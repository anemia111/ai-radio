...
export function registerRadioTools(actions: { configure: (topic: string, mode?: string, mood?: string) => void; start: () => void; stop: () => void; next: () => void; continue: () => void; read: () => object }) {
  ...
  const tools: ModelTool[] = [
    ...
    { name: 'advance_broadcast_topic', title: '次の話題へ進む', description: '現在の発話を終えて、同じテーマ内の次の話題へ進みます。', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute() { actions.next(); return { requested: 'next' } } },
    { name: 'continue_broadcast_topic', title: '話題を掘り下げる', description: '現在の発話を終えて、同じ話題をさらに掘り下げます。', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute() { actions.continue(); return { requested: 'continue' } } },
