...
- Web Speech APIによるDJ A / DJ Bの交互読み上げ、高品質な日本語音声の自動優先、個別選択と試聴
...
初期状態はDemo Modeです。テーマを入力して「放送を開始」を押すだけで動き、APIキーや外部サーバーは不要です。入力したテーマと質問意図を各セグメントの中心に置き、F1、ゲーム、AI / テクノロジー、深夜などの語を含む場合は近い会話素材を組み合わせます。最新情報やテーマ固有の詳しい回答が必要な場合はAI Modeを設定してください。AI接続・RSS・音声で問題が起きてもアプリ全体は停止せず、Demo会話または字幕で継続します。
